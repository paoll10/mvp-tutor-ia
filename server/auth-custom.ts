...
export async function loginCustom(formData: FormData) {
  ...
  try {
    ...
    // Tenta SEM schema primeiro (mais simples, menos permissões necessárias)
    ...
    console.log('Resultado sem schema:', {
      temDados: !!result.data,
      temErro: !!result.error,
      erro: result.error?.message,
      codigo: result.error?.code
    })

    // Se não encontrou sem schema, tenta com schema mentoria
    if (!result.data && result.error) {
      console.log('⚠️ Tentando com schema mentoria...')
